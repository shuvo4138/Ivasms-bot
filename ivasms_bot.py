...
async def ivasms_login():
    ...
    try:
        async with httpx.AsyncClient(follow_redirects=True, timeout=15) as client:
            ...
            csrf = ""
            match = re.search(r'name="_token"\s+value="([^"]+)"', r1.text)
            if match:
                csrf = match.group(1)
            ...
            if "dashboard" in str(r2.url) or "my_sms" in str(r2.url) or r2.status_code == 200:
                cookies = dict(client.cookies)
                _ivasms_session = {"cookies": cookies, "time": time.time()}
                logging.info("✅ ivasms login সফল!")
                return cookies
            logging.error(f"ivasms login failed: {r2.url}")
            return None
